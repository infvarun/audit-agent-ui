#!/usr/bin/env python3
"""
Complete Python backend server with FastAPI and LangChain integration
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    print("Starting Python FastAPI server with LangChain integration...")
    Base.metadata.create_all(bind=engine)
    yield
    # Shutdown
    print("Shutting down Python FastAPI server...")

# FastAPI app
app = FastAPI(
    title="Audit Data Collection API",
    description="Python FastAPI server with LangChain integration for audit data processing",
    version="1.0.0",
    lifespan=lifespan
)
...
async def health_check():
    return {"status": "healthy", "service": "python-backend", "timestamp": time.time()}
...
async def serve_spa(path: str):
    ...
    if os.path.exists("dist/public/index.html"):
        ...
        return JSONResponse({"message": "Frontend not built. Please run 'npm run build'."})
